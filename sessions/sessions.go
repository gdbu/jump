...
type Sessions struct {
	out *scribe.Scribe
	c   *mojura.Mojura[Session, *Session]
	g   *uuid.Generator
}
...
func (s *Sessions) getByKey(txn *mojura.Transaction[Session, *Session], key string) (sp *Session, err error) {
	filter := filters.Match(relationshipKeys, key)
	opts := mojura.NewFilteringOpts(filter)
	return txn.GetFirst(opts)
}

func (s *Sessions) getByUserID(txn *mojura.Transaction[Session, *Session], userID string) (ss []*Session, err error) {
...
func (s *Sessions) purge(txn *mojura.Transaction[Session, *Session], oldest int64) (err error) {
...
func (s *Sessions) invalidateUser(txn *mojura.Transaction[Session, *Session], userID string) (err error) {
	var ss []*Session
...
func (s *Sessions) Purge(oldest int64) (err error) {
	err = s.c.Transaction(context.Background(), func(txn *mojura.Transaction[Session, *Session]) (err error) {
...
func (s *Sessions) New(userID string) (key, token string, err error) {
	...
	if err = s.c.Batch(context.Background(), func(txn *mojura.Transaction[Session, *Session]) (err error) {
...
func (s *Sessions) Get(key, token string) (sp *Session, err error) {
	...
	sessionKey := makeSessionKey(key, token)
	err = s.c.ReadTransaction(context.Background(), func(txn *mojura.Transaction[Session, *Session]) (err error) {
...
func (s *Sessions) Refesh(key, token string) (err error) {
	// Create session key from the key/token pair
	sessionKey := makeSessionKey(key, token)
	err = s.c.Batch(context.Background(), func(txn *mojura.Transaction[Session, *Session]) (err error) {
		var sp *Session
		if sp, err = s.getByKey(txn, sessionKey); err != nil {
			return
		}
